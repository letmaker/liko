...
describe("Timer", () => {
  ...
  describe("计时器操作", () => {
    ...
    it("loop应添加一个循环计时器", () => {
      const callback = vi.fn();
      timer.setInterval(0.1, callback);
      expect(timer.count).toBe(1);
    });

    it("frameLoop应添加一个基于帧的循环计时器", () => {
      const callback = vi.fn();
      timer.onFrame(callback);
      expect(timer.count).toBe(1);
    });
    ...
    it("clear应清除指定的计时器", () => {
      ...
      timer.setTimeout(0.1, callback1);
      timer.setTimeout(0.2, callback2);
      expect(timer.count).toBe(2);

      timer.clearTimer(callback1);
      expect(timer.count).toBe(1);
    });
    ...
    it("clearAll(caller)应只清除指定caller的计时器", () => {
      ...
      timer.setTimeout(0.1, callback, caller1);
      timer.setTimeout(0.2, callback, caller2);
      expect(timer.count).toBe(2);
  ...
  describe("计时器执行", () => {
    ...
    it("计时器应传递参数给回调函数", () => {
      const callback = vi.fn();
      timer.setTimeout(0.1, callback, null, "arg1", "arg2");
